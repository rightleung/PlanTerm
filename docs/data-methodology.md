...
Provider adapters implement `supports`, symbol normalization and bounded fetch behavior behind a common protocol. yfinance is optional for US/HKEX/LSE; the AkShare adapter is optional and currently reports that the approved A-share live adapter is not enabled. Optional modules load lazily. Requests use a bounded thread pool/semaphore, an 8-second provider timeout, a 12-second operation deadline, bounded retries for transient provider failures, in-memory positive/negative/rate-limit caches and per-provider rate limiting. No arbitrary URL, credentialed request or unapproved scraping path is available.

Each normalized statement carries common metric IDs (`revenue`, `gross_profit`, `operating_profit`, `operating_expense`, `cost_of_sales`, `cash`, `total_assets`, `total_liabilities`, `operating_cf`) plus per-metric `flow`/`stock` semantics; the legacy row-level `is_flow` flag is retained for compatibility and is false for mixed flow/stock rows. It also carries ISO period dates, annual/quarterly type, ISO currency, declared unit scale (`native`, `thousands` or `millions`), provider, HTTPS source URL, UTC retrieval timestamp, optional as-of/filing dates and `normalization_version='public-import-v1'`. Missing or conflicting periods, non-finite values, unknown fields, malformed upstream rows and ambiguous currency become typed errors; missing values remain `null` and are never fabricated as zero. Native currency and unit are preserved and no FX conversion is performed. The preview is not numerically compared with RMB case values and has `dashboard_ready=false` because planning assumptions and cross-currency mapping are intentionally absent.

The deterministic fixture payloads are under `tests/fixtures/public_import/` and include annual and quarterly rows, native currency/scale, source metadata and anomaly cases for missing data, malformed data, duplicate/conflicting periods and currency/period errors. An opt-in live smoke test requires `PLANTERM_LIVE_PUBLIC_DATA=1` plus an explicit allowlist; it is skipped in ordinary validation.
