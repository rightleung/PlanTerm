...
export type ApiErrorCode = 'invalid_exchange' | 'invalid_venue' | 'invalid_ticker' | 'ambiguous_ticker' | 'unsupported_exchange' | 'dependency_missing' | 'no_data' | 'malformed_upstream' | 'period_inconsistent' | 'currency_missing' | 'rate_limited' | 'provider_timeout' | 'provider_unavailable' | 'validation_error' | 'internal_server_error' | 'request_too_large'
...
export interface CompanyLookupRequest { ticker: string; exchange?: 'LSE' | 'A_SHARE' | 'HKEX' | 'US'; venue?: 'SSE' | 'SZSE' | 'BSE'; data_source?: 'auto' | 'yfinance' | 'akshare' }
export interface CompanyProfile { name: string; symbol: string; exchange: string; venue?: string | null; currency?: string | null; country?: string | null; sector?: string | null; industry?: string | null; website?: string | null; description?: string | null; employees?: number | null; market_cap?: number | null; market_cap_currency?: string | null }
export interface CompanyLookupResponse { profile: CompanyProfile; source: { provider: string; url: string; retrieved_at: string; normalization_version: string }; disclosures: string[] }
export interface SymbolSearchResult { symbol: string; name: string; exchange: 'US' | 'HKEX' | 'LSE' | 'A_SHARE'; venue?: 'SSE' | 'SZSE' | 'BSE' | null; currency?: string | null; country?: string | null }
export interface SymbolSearchResponse { query: string; count: number; results: SymbolSearchResult[] }
