...
export function DataProvenance({ dashboard }: { dashboard: DashboardResponse }) {
  ...
  return (
    <section className="panel provenance-panel" aria-labelledby="provenance-title">
      ...
      <div className="provenance-grid"><div><h3>Planning assumptions</h3><div className="assumption-list">{Object.entries(assumptions.budget_assumptions).map(([unit, value]) => <div className="assumption-row" key={unit}><span>{unit}</span><span>{percent(value.revenue_growth_vs_fy2025)} growth · {percent(value.budget_gross_margin)} GM · {percent(value.budget_operating_margin)} OM · RMB {value.average_ticket} ticket</span></div>)}</div><h3>Synthetic profit allocation</h3><div className="assumption-list">{Object.entries(assumptions.profit_allocation_indices).map(([unit, value]) => <div className="assumption-row" key={unit}><span>{unit}</span><span>GM index {value.gross_margin_index.toFixed(2)} · OM index {value.operating_margin_index.toFixed(2)}</span></div>)}</div><p className="disclaimer">{assumptions.note}</p></div><div><h3>Official data sources</h3><div className="source-list">{dashboard.data_sources.map((source) => <a href={source.url} target="_blank" rel="noreferrer" key={source.url}><span>{source.name}</span><small>{source.source_date} · {source.scope}</small></a>)}</div></div></div>
